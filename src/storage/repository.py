...
from sqlalchemy import DateTime, Integer, JSON, Boolean, String, select
...
class AnalysisRecordModel(Base):
    ...
    meal_recognized: Mapped[bool] = mapped_column(
        Boolean,
        nullable=False,
        default=True,
    )
    ...
    async def get(
        ...
        async with self.SessionLocal() as session:
            ...
            return AnalysisRecord(
                ...
                ingredients=[
                    IngredientLine.model_validate(item)
                    for item in db_record.ingredients
                ],
